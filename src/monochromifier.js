...
let sketch = function (p) {
  ...
  p.setup = function () {
    ...
    displayBuffer = p.createMonochromeImage(img);
  };
  ...
  p.draw = function () {
    ...
    if (displayBuffer && dirty) {
      p.background(backgroundColor);
      // console.log(displayBuffer.width, displayBuffer.height);
  ...
  p.mouseDragged = function () {
    if (
      ...
      let scaledMouseX = p.mouseX * scaleRatio;
      let scaledMouseY = p.mouseY * scaleRatio;
      let scaledPmouseX = p.pmouseX * scaleRatio;
      let scaledPmouseY = p.pmouseY * scaleRatio;
  ...
  p.createMonochromeImage = function (img) {
    ...
    const newImg = p.createImage(scaledWidth, scaledHeight);
    newImg.copy(
      img,
      0,
      0,
    ...
    for (let y = 0; y < scaledHeight * pd; y++) {
      for (let x = 0; x < scaledWidth * pd; x++) {
        let index = (x + y * scaledWidth * pd) * 4;
    ...
    let combinedImage = p.createGraphics(scaledWidth, scaledHeight);
    combinedImage.pixelDensity(density);
    // combinedImage.imageMode(p.CENTER)
    combinedImage.image(newImg, 0, 0);
